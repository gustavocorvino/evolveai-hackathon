...
const UseCaseCard = ({ useCase, index, onClick }) => {
  // Usa isSelected se disponível (passado pela Gallery), senão usa isAvailable
  const isSelected = useCase.isSelected === true;
  const isAvailable = !isSelected && useCase.isAvailable !== false;
  ...
  return (
    <motion.div
      ...
      className={`
        ...
        ${isAvailable 
          ? 'cursor-pointer bg-neutral-dark/70 border-2 border-shield-green/50 hover:border-neon-cyan hover:shadow-glow-cyan' 
          : 'cursor-not-allowed bg-neutral-dark/30 border-2 border-neutral-light/20'}
        transition-all duration-300
      `}
      onClick={() => isAvailable && onClick && onClick()}
    >
      {/* Status Badge - só mostra "Já selecionado" quando selecionado */}
      {isSelected && (
        <div className="absolute top-4 right-4 z-10">
          <div className="px-3 py-1 bg-nova-red/20 border border-nova-red rounded-full flex items-center gap-2">
            <span className="text-nova-red text-xs font-semibold">🔒 Já selecionado</span>
          </div>
        </div>
      )}
      ...
      <div className="p-6">
        ...
        {isSelected && useCase.selectedBy && (
          <div className="pt-4 border-t border-neutral-light/10">
            <p className="text-neutral-light/50 text-xs">
              Selecionado por: <span className="text-solar-orange font-semibold">{useCase.selectedBy.teamName}</span>
            </p>
          </div>
        )}
